...
function HeatmapDrilldown({regions,primaryKey,operationHours}:{regions:CalculationRegion[];primaryKey:string;operationHours:number}) {
  ...
  const hours=mode==="reg"||operationHours===24?Array.from({length:24},(_,hour)=>hour):Array.from({length:Math.min(operationHours,15)},(_,index)=>9+index),columns=`42px repeat(${hours.length},minmax(15px,1fr))`;
  ...
  return <article className={`chart-card heatmap-card heatmap-v2 ${mode==="reg"?"regeneration":"dehumidification"}`}><header><div><span>LD OPERATION HEATMAP</span><h2>{title}</h2><small>{month===null?(mode==="abs"?"설정한 제습 운전시간만 확대 표시합니다. 월을 눌러 상세를 확인하세요.":"운전 종료 후 재생을 포함해 24시간을 표시합니다. 월을 눌러 상세를 확인하세요."):week===null?"주차를 누르면 해당 주의 월~일 가동률을 확인할 수 있습니다.":"선택 주의 월요일부터 일요일까지 시간대별 가동률입니다."}</small></div><div className="heatmap-tools"><div className="heatmap-mode"><button className={mode==="abs"?"selected":""} onClick={()=>{setMode("abs");resetDepth()}}>제습 가동률</button><button className={mode==="reg"?"selected":""} onClick={()=>{setMode("reg");resetDepth()}}>재생 가동률</button></div>{regions.length>1&&<label>지역<select value={regionKey} onChange={event=>{setRegionKey(event.target.value);resetDepth()}}>{regions.map(item=><option value={item.key} key={item.key}>{item.label.split(" · ")[0]}</option>)}</select></label>}{week!==null&&<button onClick={()=>setWeek(null)}>주차별 보기</button>}{month!==null&&<button onClick={resetDepth}>월별 보기</button>}<div className="heatmap-scale">0% <i/> 100%</div></div></header><div className="usage-heatmap"><div className="heatmap-hours" style={{gridTemplateColumns:columns}}>{hours.map(hour=><span key={hour}>{hour}시</span>)}</div>{rows.map(row=><button className="heatmap-row" style={{gridTemplateColumns:columns}} key={row.label} onClick={()=>{if(month===null)setMonth(row.month);else if(week===null&&row.week!==null)setWeek(row.week)}}><b>{row.label}</b>{hours.map(hour=>{const value=row.hours[hour]||0,ratio=Math.max(0,Math.min(1,value/100));return <i key={hour} title={`${region.label.split(" · ")[0]} · ${row.label} ${hour}시 · ${mode==="abs"?"제습":"재생"} 가동률 ${value.toFixed(1)}%`} style={{backgroundColor:mode==="abs"?`hsl(${188+ratio*38} 88% ${74-ratio*30}%)`:`hsl(${8-ratio*8} 82% ${78-ratio*34}%)`,opacity:.14+ratio*.86}}/>})}</button>)}</div></article>;
}
...
function WeatherChart({region,regions,selectedKey,onSelect}:{region:CalculationRegion;regions:CalculationRegion[];selectedKey:string;onSelect:(key:string)=>void}) {
  ...
  return <article className="chart-card weather-card"><header><div><span>WEATHER &amp; SOLAR INPUT</span><h2>월별 날씨 및 일사 데이터</h2><small>계열별 독립 스케일을 사용한 월별 상대 추세 · 마우스를 올리면 실제값을 확인할 수 있습니다.</small></div>{regions.length>1&&<label className="weather-region-select">지역<select value={selectedKey} onChange={e=>onSelect(e.target.value)}>{regions.map(item=><option value={item.key} key={item.key}>{item.label.split(" · ")[0]}</option>)}</select></label>}</header><div className="weather-combo-legend"><span><i className="temp"/><span>외기온도<small>월평균</small></span><b>{tb.min.toFixed(1)}–{tb.max.toFixed(1)} °C</b></span><span><i className="humidity"/><span>절대습도<small>월평균</small></span><b>{hb.min.toFixed(1)}–{hb.max.toFixed(1)} g/kgDA</b></span><span><i className="solar"/><span>집열면 일사량<small>월 누적</small></span><b>{sb.min.toFixed(0)}–{sb.max.toFixed(0)} kWh/m²</b></span></div><div className="weather-combo-plot"><div className="weather-y-ticks" aria-hidden="true"><span>100</span><span>75</span><span>50</span><span>25</span><span>0</span></div><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-label={`${region.label} 월별 날씨 및 일사 복합 그래프`}>{[12,31,50,69,88].map(v=><line key={v} x1="0" x2="100" y1={v} y2={v} className="weather-gridline"/>)}{solar.map((value,index)=>{const width=rows.length?55/rows.length:5;const barY=y(value,sb);return <rect key={index} x={x(index)-width/2} y={barY} width={width} height={88-barY} className="weather-solar-bar"><title>{rows[index]?.month}월 · 일사량 {value.toFixed(1)} kWh/m²</title></rect>})}<polyline points={tempPoints} className="weather-temp-line"/><polyline points={humidityPoints} className="weather-humidity-line"/>{temps.map((value,index)=><circle key={`t${index}`} cx={x(index)} cy={y(value,tb)} r="1.2" className="weather-temp-dot"><title>{rows[index]?.month}월 · 외기온도 {value.toFixed(1)} °C</title></circle>)}{humidity.map((value,index)=><circle key={`h${index}`} cx={x(index)} cy={y(value,hb)} r="1.2" className="weather-humidity-dot"><title>{rows[index]?.month}월 · 절대습도 {value.toFixed(1)} g/kgDA</title></circle>)}</svg><div className="weather-month-axis">{rows.map(row=><span key={row.month}>{row.month}월</span>)}</div></div></article>;
}
...
function ConcentrationChart({monthly,drilldown,region}:{monthly:Array<Record<string,number|string>>;drilldown:ConcentrationMonth[];region:string}) {
  ...
  return <article className="chart-card concentration-card"><header><div><span>LD SOLUTION STATE · {region.split(" · ")[0]}</span><h2>{title}</h2><small>{description}</small></div><div className="concentration-tools">{selectedDay!==null&&<button onClick={()=>setSelectedDay(null)}>← 일별 보기</button>}{selectedMonth!==null&&<button onClick={()=>{setSelectedMonth(null);setSelectedDay(null)}}>← 월별 보기</button>}<div className="concentration-legend"><i/>평균 농도 <span/>최소–최대 범위</div></div></header><div className="concentration-layout"><div className="concentration-y-title">LiCl 농도 (wt%)</div><div className="concentration-y-axis">{ticks.map(value=><span key={value} style={{top:`${y(value)}%`}}>{value.toFixed(1)}</span>)}</div><div className="concentration-plot"><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-label={`${region} ${title}`}>{ticks.map(value=><line key={value} x1="0" x2="100" y1={y(value)} y2={y(value)} className="concentration-grid"/>)}{rows.map((item,index)=><line key={item.key} x1={x(index)} x2={x(index)} y1={y(item.max)} y2={y(item.min)} className="concentration-range"><title>{item.label} · {item.min.toFixed(2)}–{item.max.toFixed(2)} wt%</title></line>)}<polyline points={meanPoints} className="concentration-line"/>{rows.map((item,index)=><circle tabIndex={selectedDay===null?0:undefined} role={selectedDay===null?"button":undefined} onClick={()=>selectPoint(item.key)} onKeyDown={event=>{if(event.key==="Enter"||event.key===" ")selectPoint(item.key)}} key={item.key} cx={x(index)} cy={y(item.mean)} r="1.4" className={`concentration-dot${selectedDay===null?" clickable":""}`}><title>{item.label} · 평균 {item.mean.toFixed(2)} wt%</title></circle>)}</svg><div className={`concentration-x-axis axis-${selectedMonth===null?"months":selectedDay===null?"days":"hours"}`} style={{gridTemplateColumns:`repeat(${rows.length},minmax(32px,1fr))`}}>{rows.map(item=><button disabled={selectedDay!==null} onClick={()=>selectPoint(item.key)} key={item.key}>{item.label}</button>)}</div><div className="concentration-x-title">{selectedMonth===null?"월":selectedDay===null?"일":"시각"}</div></div></div></article>;
}
