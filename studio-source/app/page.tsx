...
function InlineDetail({type,region,onClose}:{type:"dehum"|"unmet"|"area";region:CalculationRegion;onClose:()=>void}) {
  ...
  if(type==="unmet") {
    ...
    return <section className="inline-detail"><DetailHeader index="02" title="목표 제습 미충족 추이" region={region.label} onClose={onClose}/><div className="trace-action"><div><p id="trace-help">아래 점 또는 표의 행을 선택한 뒤 버튼을 누르면, 해당 한 시간의 농도·급기 습도·제습 및 재생 상태를 60초 단위로 계산합니다. 여러 점을 선택하면 마지막으로 선택한 시점을 분석합니다.</p><strong aria-live="polite">{selectedTime?"선택 시점: "+selectedTime.slice(5)+" · "+region.label:"먼저 분석할 점을 선택하세요."}</strong></div><button type="button" disabled={!selectedTime} aria-describedby="trace-help" onClick={()=>setTraceTime(selectedTime)}>선택 시점의 60초별 상세 계산</button></div><div className="unmet-summary"><div><b>{n(trend.totalHours).toFixed(1)} h</b><small>상한 초과시간</small></div><div><b>{n(trend.averageHumidityExcess).toFixed(2)} g/kgDA</b><small>평균 초과량</small></div><div><b>{n(trend.maxHumidityExcess).toFixed(2)} g/kgDA</b><small>최대 초과량</small></div></div><div className="unmet-chart"><div className="unmet-y-title">급기 절대습도 (g/kgDA)</div><div className="unmet-y-axis">{ticks.map(tick=><span key={tick} style={{bottom:`${y(tick)}%`}}>{tick.toFixed(1)}</span>)}</div><div className="unmet-points">{ticks.map(tick=><em key={tick} style={{bottom:`${y(tick)}%`}}/>)}{events.map((item,index)=><button type="button" key={index} className={selectedUnmetEvents.has(index)?"selected":""} aria-pressed={selectedUnmetEvents.has(index)} aria-label={`${item.time} 급기 절대습도 ${n(item.supplyHumidity).toFixed(2)} g/kgDA`} title={`${item.time} · ${n(item.supplyHumidity).toFixed(2)} g/kgDA`} onClick={()=>toggleUnmetEvent(index)} style={{left:`${x(index)}%`,bottom:`${y(n(item.supplyHumidity))}%`}}/>)}<span className="unmet-upper-line" style={{bottom:`${y(accepted)}%`}}>허용상한 {accepted.toFixed(1)} g/kgDA</span><span className="unmet-average-line" style={{bottom:`${y(averageSupply)}%`}}>미충족 평균 {averageSupply.toFixed(2)} g/kgDA</span></div><div className="unmet-x-axis">{xIndexes.map(index=><span key={index} style={{left:`${x(index)}%`}}>{String(events[index]?.time||"").slice(5,16)}</span>)}</div><div className="unmet-x-title">발생 시각 (월-일 시:분)</div></div>{traceTime&&<SubstepTrace key={traceTime} request={region.traceRequest} time={traceTime}/>}<div className="event-table event-table-detailed"><div><b>발생시각</b><b>외기조건</b><b>LiCl 농도</b><b>급기 절대습도</b><b>상한 초과</b></div>{events.map((item,index)=><button type="button" key={index} className={selectedUnmetEvents.has(index)?"selected":""} aria-pressed={selectedUnmetEvents.has(index)} onClick={()=>toggleUnmetEvent(index)}><span>{String(item.time||"").slice(5)}</span><span className="outdoor-condition"><b>{n(item.outdoorTemp).toFixed(1)} °C</b><small>{n(item.outdoorHumidity).toFixed(2)} g/kgDA</small></span><span>{eventConcentration(item)>0?`${eventConcentration(item).toFixed(2)} wt%`:"–"}</span><span>{n(item.supplyHumidity).toFixed(2)} g/kgDA</span><span>+{n(item.humidityExcess).toFixed(2)}</span></button>)}</div></section>;
  }
