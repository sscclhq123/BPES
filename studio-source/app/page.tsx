...
export default function Home() {
  ...
  const ldRangeError = design.solutionConcentration<36.4||design.solutionConcentration>39||design.lgRatio<1||design.lgRatio>3||design.absSolutionTemp<20||design.absSolutionTemp>31.4||design.regenTemp<48.5||design.regenTemp>59.4||design.regenLgRatio<.65||design.regenLgRatio>2;
  ...
  const appUrl = useMemo(() => {
    ...
    if(design.ldDesignMode==="auto"){params.set("regenLgRatio","1.2");}
...
function SystemOverview({onBack,onNext,reset}:{onBack:()=>void;onNext:()=>void;reset:()=>void}) {
  ...
  return <main className="entry overview-screen"><Logo reset={reset}/><section className="system-overview screen-reveal"><header><div><span>SYSTEM DEFINITION</span><h2>SYSTEM SCHEMATIC</h2></div><small>INTRODUCTION · 00 / 04</small></header>
    <div className="system-scene" aria-label="SALDDP 시스템 구성도">
      ...
      {activeDetail===2&&<LdSchematic onClose={()=>setActiveDetail(null)}/>}
...
function StepFields({step,design,update,weatherFile,onWeatherFile}:{step:number;design:Design;update:<K extends keyof Design>(key:K,value:Design[K])=>void;weatherFile:File|null;onWeatherFile:(file:File|null)=>void}) {
  ...
  if(step===2) {
    const selectMode=(mode:Design["ldDesignMode"])=>{update("ldDesignMode",mode);if(mode==="auto"){update("lgMode","auto");update("absTempMode","auto");update("regenMode","auto");update("regenLgRatio",1.2);}};
    return <div className="ld-fields"><section className="ld-requirements"><b>USER REQUIREMENTS</b><h3>목표 제습조건</h3><div className="field-grid"><NumberField label="목표 급기 절대습도" unit="g/kgDA" value={design.targetAbsHumidity} step={.1} onChange={v=>update("targetAbsHumidity",v)}/><NumberField label="허용상한 여유" unit="g/kgDA" value={design.targetHumidityTolerance} step={.1} onChange={v=>update("targetHumidityTolerance",v)}/></div></section><div className="ld-design-mode"><button type="button" className={design.ldDesignMode==="auto"?"selected":""} onClick={()=>selectMode("auto")}><b>PROGRAM SIZING</b><strong>운전조건 자동 산정</strong><small>실험식 범위에서 목표 습도를 만족하는 L/G와 용액온도를 계산합니다.</small></button><button type="button" className={design.ldDesignMode==="manual"?"selected":""} onClick={()=>selectMode("manual")}><b>USER DEFINED</b><strong>사용자 운전조건 적용</strong><small>사용자가 지정한 조건을 고정하거나 선택 변수만 자동 조정합니다.</small></button></div>{design.ldDesignMode==="auto"?<div className="ld-auto-scope"><span>AUTO DESIGN RANGE</span>
<h4>제습부 · 프로그램 적용범위</h4><div className="ld-range-grid"><p><b>공통 탱크 LiCl</b>36.4–39.0 wt%</p><p><b>제습 L/G</b>1.0–3.0</p><p><b>제습 용액온도</b>20.0–31.4 °C</p></div>
<h4>재생부 · 상관식 실험범위</h4><div className="ld-range-grid"><p><b>공기유량 / 모듈</b>0.24–0.40 kg/s</p><p><b>용액유량 / 모듈</b>0.26–0.48 kg/s</p><p><b>입구 용액온도</b>48.5–59.4 °C</p><p><b>입구 LiCl 농도</b>23.7–40.7 wt%</p><p><b>입구 외기온도</b>30.3–36.6 °C</p><p><b>입구 절대습도</b>10.5–22.0 g/kgDA</p></div>
<small>공통 탱크 농도는 제습부 범위에 맞춰 더 좁게 제한합니다. 외기조건이 실험범위를 벗어나는 시간은 범위 내 검증으로 간주할 수 없습니다.</small>
<p className="ld-auto-budget"><b>자동 설계 가정</b>재생 L/G 1.2 고정 · <strong>필요 재생 용량 자동 산정</strong><br/>제습 풍량 대비 임의의 배수 제한 없이 모듈 수를 산정합니다. 운전 중에는 산정된 설비 용량 안에서만 변풍량 제어합니다.</p>
<small>L/G = 용액 질량유량 / 공기 질량유량. 재생 변풍량 제어는 제습 설계 풍량 1배부터 산정된 최대 풍량까지(가동 중) 고정 L/G로 팬·용액 펌프를 연동합니다. 모듈별 유량 교집합을 지키며 저부하는 가동시간으로 조절합니다. 재생 대수는 목표 농도에서의 수분 배출 요구량으로 산정됩니다.</small></div>:<><div className="field-grid"><NumberField label="LiCl 초기 농도" unit="%" value={design.solutionConcentration} step={.1} min={36.4} max={39} onChange={v=>update("solutionConcentration",v)}/><NumberField label="초기 L/G" unit="-" value={design.lgRatio} step={.1} min={1} max={3} onChange={v=>update("lgRatio",v)}/><NumberField label="제습부 용액온도" unit="°C" value={design.absSolutionTemp} step={.1} min={20} max={31.4} onChange={v=>update("absSolutionTemp",v)}/><NumberField label="재생기 용액온도" unit="°C" value={design.regenTemp} step={.1} min={48.5} max={59.4} onChange={v=>update("regenTemp",v)}/></div><div className="ld-mode-grid"><label>L/G 제어<select value={design.lgMode} onChange={e=>update("lgMode",e.target.value as Design["lgMode"])}><option value="auto">자동조정</option><option value="fixed">입력값 고정</option></select></label><label>제습부 온도 제어<select value={design.absTempMode} onChange={e=>update("absTempMode",e.target.value as Design["absTempMode"])}><option value="auto">자동조정</option><option value="fixed">입력값 고정</option></select></label><label>재생기 온도 제어<select value={design.regenMode} onChange={e=>update("regenMode",e.target.value as Design["regenMode"])}><option value="auto">자동조정</option><option value="fixed">입력값 고정</option></select></label></div><p className="ld-guidance"><b>제어 순서</b> L/G → 제습부 용액온도 → 재생기 용액온도. 고정으로 선택한 변수는 반복계산 중 변경하지 않습니다.</p></>}{design.ldDesignMode==="manual"&&<section style={{marginTop:20,paddingTop:16,borderTop:"1px solid #48534d"}}><h3>재생부 독립 용량 산정</h3><div className="field-grid"><NumberField label="재생부 고정 L/G" unit="kg/kg" value={design.regenLgRatio} step={.05} min={.65} max={2} onChange={v=>update("regenLgRatio",v)}/></div><p style={{fontSize:14,lineHeight:1.7,color:"#c6d1ca"}}>목표 농도에서 필요한 수분 배출량으로 재생 대수를 산정합니다. 고정 L/G를 유지하며 재생 팬과 용액 펌프 유량을 함께 조절합니다. 제습부 풍량은 건물 필요 도입량으로 유지합니다. 모듈당 공기 0.24–0.40 kg/s, 용액 0.26–0.48 kg/s의 교집합 안에서만 운전합니다.</p><p style={{fontSize:13,lineHeight:1.6,color:"#c6d1ca"}}>가동 중 재생 풍량은 제습 설계 풍량 1배부터 부하로 산정된 설비의 최대 풍량까지 변풍량으로 조절합니다. 저부하는 최소 풍량 이하로 낮추지 않고 가동시간을 조절하므로 시간평균 유량은 1배보다 작을 수 있습니다. 1배 하한은 설계 가정이며 모듈별 실험식 범위는 유지합니다. 설치 대수는 설계 시 확정하며 운전 중 추가되지 않습니다. 재생 불가 외기조건이나 용량 부족은 별도로 표시합니다. 팬 300 Pa·효율 60%, 덕트 5 m/s를 가정한 규모 추정치를 결과에 제공합니다(실제 기기 선정 아님).</p></section>}</div>;
  }
...
function ResultOverview({summary,design,onReset}:{summary:CalculationSummary;design:Design;onReset:()=>void}) {
  ...
  return <main className="result-screen">
    ...
    <section className="result-dashboard">
      ...
      {/* Sibling chart keys must be unique, even when both reset on a region change. */}
      ...
      <LdPerformance key={`performance:${primary.key}`} data={primary.ldPerformance} region={primary.label}/>
...
function GeographicAnalysis({regions,selectedKey,onSelect,buildingArea}:{regions:CalculationRegion[];selectedKey:string;onSelect:(key:string)=>void;buildingArea:number}) {
  ...
  const areaPercent=(region:CalculationRegion)=>{const base=Number(region.traceRequest?.buildingArea)||buildingArea;const area=Number(region.best.collectorArea);return base>0&&Number.isFinite(area)?`${(area/base*100).toFixed(1)}%`:"—";};
