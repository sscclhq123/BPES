...
function WeatherChart({region,regions,selectedKey,onSelect}:{region:CalculationRegion;regions:CalculationRegion[];selectedKey:string;onSelect:(key:string)=>void}) {
  ...
  return <article className="chart-card weather-card"><header><div><span>WEATHER &amp; SOLAR INPUT</span><h2>월별 날씨 및 일사 데이터</h2><small>단위가 다른 세 계열을 한 화면에서 비교하기 위해 각 계열의 연중 최솟값–최댓값을 0–100%로 환산했습니다.</small></div>{regions.length>1&&<label className="weather-region-select">지역<select value={selectedKey} onChange={e=>onSelect(e.target.value)}>{regions.map(item=><option value={item.key} key={item.key}>{item.label.split(" · ")[0]}</option>)}</select></label>}</header><div className="weather-combo-legend"><span><i className="temp"/><span>외기온도<small>월평균 · 선</small></span><b>{tb.min.toFixed(1)}–{tb.max.toFixed(1)} °C</b></span><span><i className="humidity"/><span>절대습도<small>월평균 · 선</small></span><b>{hb.min.toFixed(1)}–{hb.max.toFixed(1)} g/kgDA</b></span><span><i className="solar"/><span>집열면 일사량<small>월 누적 · 막대</small></span><b>{sb.min.toFixed(0)}–{sb.max.toFixed(0)} kWh/m²</b></span></div><div className="weather-axis-layout"><div className="weather-y-title">연중 상대 위치 (%)</div><div className="weather-y-ticks" aria-hidden="true"><span>100</span><span>75</span><span>50</span><span>25</span><span>0</span></div><div className="weather-combo-plot"><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-label={`${region.label} 월별 날씨 및 일사 복합 그래프`}><defs><linearGradient id="weatherSolarFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#ff8279" stopOpacity=".42"/><stop offset="1" stopColor="#ff8279" stopOpacity=".04"/></linearGradient></defs>{[12,31,50,69,88].map(v=><line key={v} x1="0" x2="100" y1={v} y2={v} className="weather-gridline"/>)}{rows.map((_,index)=><line key={`v${index}`} x1={x(index)} x2={x(index)} y1="12" y2="88" className="weather-gridline weather-gridline-v"/>)}{solar.map((value,index)=>{const width=rows.length?32/rows.length:3;const barY=y(value,sb);return <rect key={index} x={x(index)-width/2} y={barY} width={width} height={88-barY} className="weather-solar-bar"><title>{rows[index]?.month}월 · 일사량 {value.toFixed(1)} kWh/m²</title></rect>})}<polyline points={tempPoints} className="weather-temp-line"/><polyline points={humidityPoints} className="weather-humidity-line"/>{temps.map((value,index)=><circle key={`t${index}`} cx={x(index)} cy={y(value,tb)} r="1.35" className="weather-temp-dot"><title>{rows[index]?.month}월 · 외기온도 {value.toFixed(1)} °C</title></circle>)}{humidity.map((value,index)=><circle key={`h${index}`} cx={x(index)} cy={y(value,hb)} r="1.35" className="weather-humidity-dot"><title>{rows[index]?.month}월 · 절대습도 {value.toFixed(1)} g/kgDA</title></circle>)}</svg><div className="weather-month-axis">{rows.map(row=><span key={row.month}>{row.month}월</span>)}</div><div className="weather-x-title">월</div></div></div></article>;
}
