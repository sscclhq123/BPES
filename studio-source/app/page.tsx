...
export default function Home() {
  ...
  const ldRangeError = design.solutionConcentration<36.4||design.solutionConcentration>39||design.lgRatio<1||design.lgRatio>3||design.absSolutionTemp<20||design.absSolutionTemp>31.4||design.regenTemp<48.5||design.regenTemp>59.4||design.regenLgRatio<.65||design.regenLgRatio>2||design.regenMaxAirRatio<1||design.regenMaxAirRatio>3;
  ...
  const appUrl = useMemo(() => {
    ...
    if(design.ldDesignMode==="auto"){params.set("regenLgRatio","1.2");params.set("regenMaxAirRatio","3");}
...
function SystemOverview({onBack,onNext,reset}:{onBack:()=>void;onNext:()=>void;reset:()=>void}) {
  ...
  return <main className="entry overview-screen"><Logo reset={reset}/><section className="system-overview screen-reveal"><header><div><span>SYSTEM DEFINITION</span><h2>SYSTEM SCHEMATIC</h2></div><small>INTRODUCTION · 00 / 04</small></header>
    <div className="system-scene" aria-label="SALDDP 시스템 구성도">
      ...
      {portalReady&&activeDetail===2&&createPortal(<LdSchematic onClose={()=>setActiveDetail(null)}/>,document.body)}
...
function StepFields({step,design,update,weatherFile,onWeatherFile}:{step:number;design:Design;update:<K extends keyof Design>(key:K,value:Design[K])=>void;weatherFile:File|null;onWeatherFile:(file:File|null)=>void}) {
  ...
  if(step===2) {
    const selectMode=(mode:Design["ldDesignMode"])=>{update("ldDesignMode",mode);if(mode==="auto"){update("lgMode","auto");update("absTempMode","auto");update("regenMode","auto");update("regenLgRatio",1.2);update("regenMaxAirRatio",3)}};
...
function ResultOverview({summary,design,onReset}:{summary:CalculationSummary;design:Design;onReset:()=>void}) {
  ...
  return <main className="result-screen">
    ...
    <section className="result-dashboard">
      ...
      <LdFlowDiagnostics key={primary.key} data={primary.ldFlowHourly} region={primary.label} reference={Number(best.regenFixedLg)||undefined} request={primary.traceRequest}/>
