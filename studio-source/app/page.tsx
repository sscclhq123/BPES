...
function ConcentrationChart({monthly,drilldown,region}:{monthly:Array<Record<string,number|string>>;drilldown:ConcentrationMonth[];region:string}) {
  ...
  return <article className="chart-card concentration-card"><header><div><span>LD SOLUTION STATE · {region.split(" · ")[0]}</span><h2>{title}</h2><small>{description}</small></div><div className="concentration-tools">{selectedDay!==null&&<button onClick={()=>setSelectedDay(null)}>← 일별 보기</button>}{selectedMonth!==null&&<button onClick={()=>{setSelectedMonth(null);setSelectedDay(null)}}>← 월별 보기</button>}<div className="concentration-legend"><i/>평균 농도 <span/>최소–최대 범위</div></div></header><div className="concentration-layout"><div className="concentration-y-title">LiCl 농도 (wt%)</div><div className="concentration-y-axis">{ticks.map(value=><span key={value} style={{top:`${y(value)}%`}}>{value.toFixed(1)}</span>)}</div><div className="concentration-plot" style={{minWidth:plotWidth}}><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-label={`${region} ${title}`}>{ticks.map(value=><line key={value} x1="0" x2="100" y1={y(value)} y2={y(value)} className="concentration-grid"/>)}<polygon points={bandPoints} className="concentration-band"/><polyline points={meanPoints} className="concentration-line"/>{rows.map((item,index)=><circle tabIndex={selectedDay===null?0:undefined} role={selectedDay===null?"button":undefined} onClick={()=>selectPoint(item.key)} onKeyDown={event=>{if(event.key==="Enter"||event.key===" ")selectPoint(item.key)}} key={item.key} cx={x(index)} cy={y(item.mean)} r="1.4" className={`concentration-dot${selectedDay===null?" clickable":""}`}><title>{item.label} · 평균 {item.mean.toFixed(2)} wt% · 범위 {item.min.toFixed(2)}–{item.max.toFixed(2)} wt%</title></circle>)}</svg><div className={`concentration-x-axis axis-${selectedMonth===null?"months":selectedDay===null?"days":"hours"}`} style={{gridTemplateColumns:`repeat(${rows.length},minmax(32px,1fr))`}}>{rows.map(item=><button disabled={selectedDay!==null} onClick={()=>selectPoint(item.key)} key={item.key}><i/>{item.label}</button>)}</div><div className="concentration-x-title">{selectedMonth===null?"월":selectedDay===null?"일 (날짜)":"시각 (24시간)"}</div></div></div></article>;
}
