...
type Design = {
  ...
  regenLgRatio:number; regenMaxAirRatio:number;
...
const initialDesign: Design = {
  ...
  regenLgRatio:1.2, regenMaxAirRatio:3,
...
export default function Home() {
  ...
  const ldRangeError = design.solutionConcentration<36.4||design.solutionConcentration>39||design.lgRatio<1||design.lgRatio>3||design.absSolutionTemp<20||design.absSolutionTemp>31.4||design.regenTemp<48.5||design.regenTemp>59.4||design.regenLgRatio<.65||design.regenLgRatio>2||design.regenMaxAirRatio<.5||design.regenMaxAirRatio>10;
...
function SystemOverview({onBack,onNext,reset}:{onBack:()=>void;onNext:()=>void;reset:()=>void}) {
  const schematicDetails = [
    {no:"01",label:"외기·기상",title:"ASHRAE Climate Zone 대표지역",body:"ASHRAE Standard 169-2021의 열·습윤 기후구분을 기준으로 대표도시를 구성했습니다. 국내 주요 도시는 별도로 유지하고, 해외 비교군은 0A부터 5A까지 중복되지 않게 선택했습니다."},
  ...
  return <main className="entry overview-screen"><Logo reset={reset}/><section className="system-overview screen-reveal"><header><div><span>SYSTEM DEFINITION</span><h2>SYSTEM SCHEMATIC</h2></div><small>INTRODUCTION · 00 / 04</small></header>
    <div className="system-scene" aria-label="SALDDP 시스템 구성도">
      ...
      <div className={`scene-canvas${activeDetail===0?" weather-detail-open":""}${activeDetail===1?" building-detail-open":""}${activeDetail===2?" ld-detail-open":""}${activeDetail===3?" collector-detail-open":""}`} onClickCapture={(event)=>{if((event.target as HTMLElement).closest(".ld-mini-close,.collector-mini-close"))window.setTimeout(()=>setActiveDetail(null),0)}}>
      ...
      {schematicDetails.map((item,index)=><button key={item.no} className={`scene-hotspot hotspot-${["weather","building","ld","solar","tes"][index]}${activeDetail===index?" selected":""}`} aria-label={`${item.no} ${item.label} 상세 보기`} aria-expanded={activeDetail===index} onClick={()=>setActiveDetail(activeDetail===index?null:index)}><b>{item.no}</b><em>{item.label}</em></button>)}
      {activeDetail===0&&<aside className="weather-mini-schematic" aria-label="ASHRAE Climate Zone 대표지역 설명"><button type="button" className="weather-mini-close" aria-label="기후대 설명 닫기" onClick={()=>setActiveDetail(null)}>×</button><header><span>ASHRAE STANDARD 169-2021</span><b>Climate Zone 대표지역</b><small>숫자는 열적 기후구분, A는 습윤(Humid) 조건을 뜻합니다. SALDDP는 제습·냉방 비교를 위해 습윤 기후대의 대표도시를 단계별로 구성합니다.</small></header><div className="climate-zone-grid"><article><b>0A</b><span>EXTREMELY HOT · HUMID</span><strong>마닐라</strong></article><article><b>1A</b><span>VERY HOT · HUMID</span><strong>하노이</strong></article><article><b>2A</b><span>HOT · HUMID</span><strong>휴스턴</strong></article><article><b>3A</b><span>WARM · HUMID</span><strong>상하이</strong></article><article><b>4A</b><span>MIXED · HUMID</span><strong>서울</strong></article><article><b>5A</b><span>COOL · HUMID</span><strong>베를린</strong></article></div><p className="weather-note"><b>국내 도시 제공 범위</b> 서울·대구·인천·광주·대전·울산은 4A, 부산·제주는 3A입니다. 화면의 기후대는 각 TMYx 파일에 포함된 ASHRAE 169-2021 메타데이터를 사용합니다.</p></aside>}
      {activeDetail===1&&<aside className="building-mini-schematic" aria-label="건물 모델과 최소 외기도입량 산정 근거"><button type="button" className="building-mini-close" aria-label="건물 상세 닫기" onClick={(event)=>{event.stopPropagation();setActiveDetail(null);}}>×</button><header><span>BUILDING MODEL &amp; OUTDOOR AIR BASIS</span><b>건물 모델·최소 외기도입량</b><small>건물 형상·재실조건과 용도별 외기 기준을 결합해 LD 외조기의 설계 처리풍량을 산정합니다.</small></header><div className="building-basis-flow"><article><em>DOE PROTOTYPE</em><h4>건물 조건</h4><p><b>A<sub>z</sub></b> 존 바닥면적</p><p><b>P<sub>z</sub></b> 설계 재실인원</p><p>용도별 재실·운전 스케줄</p></article><i>+</i><article><em>ASHRAE 62.1</em><h4>용도별 외기 기준</h4><p><b>R<sub>p</sub></b> 인원당 외기량</p><p><b>R<sub>a</sub></b> 면적당 외기량</p><p>공간 용도별 서로 다른 기준값</p></article><i>→</i><article className="building-result"><em>SALDDP · DOAS</em><h4>최소 외기도입량</h4><strong>V<sub>bz</sub> = R<sub>p</sub>P<sub>z</sub> + R<sub>a</sub>A<sub>z</sub></strong><p>전체 외기를 LD 병렬 모듈의 설계 처리풍량으로 사용</p></article></div><div className="building-use-list"><span>OFFICE</span><span>RETAIL</span><span>RESIDENTIAL</span><span>HOSPITAL</span><span>INDUSTRIAL</span><span>AGRICULTURE</span></div><p className="building-note"><b>현재 프로그램 산정 범위</b> 위 식은 존 호흡영역 외기량의 기본 구조입니다. 완전한 62.1 시스템 환기량 계산에서는 공기분배 유효도(E<sub>z</sub>)와 다중존 시스템 환기효율(E<sub>v</sub>)을 추가 검토해야 합니다.</p></aside>}
      {portalReady&&activeDetail===2&&createPortal(<aside className="ld-mini-schematic ld-shared-tank" aria-label="LD 제습 및 재생 내부 물질·열 흐름"><button type="button" className="ld-mini-close" aria-label="LD 상세 스케메틱 닫기" onClick={(event)=>{event.stopPropagation();setActiveDetail(null);}}>×</button><header><span>SHARED SOLUTION TANK · TWO LOOPS</span><b>공통 수용액탱크와 제습·재생 순환</b><small>하단의 한 탱크에서 각각 공급하고, 두 환수액이 다시 혼합됩니다. TES 열매체와 LiCl 용액은 섞이지 않고 열만 교환합니다.</small></header>
      <div className="ld-tank-diagram" tabIndex={0} role="region" aria-label="LD 순환 흐름도, 작은 화면에서는 좌우로 스크롤"><svg viewBox="0 0 900 600" role="img" aria-label="공통 수용액탱크에서 왼쪽 냉각기를 거쳐 제습부로, 오른쪽 가열 열교환기를 거쳐 재생부로 공급하고 각각 환수하는 병렬 순환">
        <defs>{[["Solution","#9ee678"],["Air","#ffd24c"],["Cold","#4ee7f0"],["Heat","#ff6d64"]].map(([id,color])=><marker key={id} id={`tank${id}`} markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto"><path d="M0 0L7 3.5L0 7Z" fill={color}/></marker>)}</defs>
        <g className="ld-unit absorber"><rect x="140" y="70" width="220" height="190" rx="14"/><text className="unit-title" x="250" y="105">ABSORBER</text><text className="unit-sub" x="250" y="134">제습부</text><path className="packing" d="M174 156H326M174 176H326M174 196H326"/><text className="transfer-copy" x="250" y="234">수분: 공기 → 용액</text></g>
        <g className="ld-unit regenerator"><rect x="540" y="70" width="220" height="190" rx="14"/><text className="unit-title" x="650" y="105">REGENERATOR</text><text className="unit-sub" x="650" y="134">재생부</text><path className="packing" d="M574 156H726M574 176H726M574 196H726"/><text className="transfer-copy" x="650" y="234">수분: 용액 → 공기</text></g>
        <path className="tank-pipe air" d="M20 178H135" markerEnd="url(#tankAir)"/><text x="75" y="151">외기 유입</text>
        <path className="tank-pipe cold" d="M365 178H458" markerEnd="url(#tankCold)"/><text x="417" y="151">건조 급기</text><text className="tank-note" x="417" y="208">건물로 공급</text>
        <path className="tank-pipe air" d="M880 160H765" markerEnd="url(#tankAir)"/><text x="827" y="134">외기 유입</text>
        <path className="tank-pipe heat" d="M765 220H880" markerEnd="url(#tankHeat)"/><text x="827" y="199">습윤 배기</text>
        <path className="tank-pipe solution" d="M340 510H80V330H180V265" markerEnd="url(#tankSolution)"/>
        <path className="tank-pipe solution" d="M560 510H820V330H640V265" markerEnd="url(#tankSolution)"/>
        <path className="tank-pipe solution return" d="M310 265V426H390V455" markerEnd="url(#tankSolution)"/>
        <path className="tank-pipe solution return" d="M570 265V426H510V455" markerEnd="url(#tankSolution)"/>
        <text className="solution-copy" x="220" y="353">냉각 용액 공급 ↑</text><text className="solution-copy" x="760" y="302">가열 용액 공급 ↑</text>
        <text className="solution-copy" x="360" y="352">희석 환수 ↓</text><text className="solution-copy" x="517" y="380">농축 환수 ↓</text>
        <g className="tank-hx cold"><rect x="25" y="355" width="110" height="70" rx="10"/><text x="80" y="384">용액 냉각</text><text className="tank-note" x="80" y="408">냉각 열교환기</text></g>
        <g className="tank-hx heat"><rect x="765" y="355" width="110" height="70" rx="10"/><text x="820" y="384">용액 가열</text><text className="tank-note" x="820" y="408">가열 열교환기</text></g>
        <path className="tank-pipe cold" d="M139 395H218" markerEnd="url(#tankCold)"/><text className="tank-note" x="193" y="374">냉각열 제거</text>
        <path className="tank-pipe heat" d="M676 395H759" markerEnd="url(#tankHeat)"/><text className="tank-note" x="686" y="349">태양열 → TES</text><text className="tank-note" x="686" y="373">+ 보조열원</text>
        <g className="common-solution-tank"><path d="M340 475V558C340 584 560 584 560 558V475Z"/><ellipse cx="450" cy="475" rx="110" ry="18"/><text className="unit-title" x="450" y="516">SOLUTION TANK</text><text x="450" y="543">공통 LiCl 수용액탱크</text><text className="tank-note" x="450" y="565">완전 혼합 · 농도 / 온도 공유</text></g>
        <text className="tank-note" x="195" y="540">탱크 → 제습 순환</text><text className="tank-note" x="705" y="540">탱크 → 재생 순환</text>
      </svg></div><div className="ld-mini-legend"><span className="solution">실선: 용액 공급</span><span className="solution">점선: 용액 환수</span><span className="dry">냉각열 제거</span><span className="heat">재생열 공급</span></div><p className="ld-tank-help">두 장치는 직렬 연결이 아닙니다. 제습부 환수와 재생부 환수가 공통 탱크에서 혼합된 뒤 각 루프로 다시 공급됩니다. 작은 화면에서는 흐름도를 좌우로 밀어 확인하세요.</p></aside>,document.body)}
      {activeDetail===3&&<aside className="collector-mini-schematic" aria-label="태양열 집열기 종류 비교"><button type="button" className="collector-mini-close" aria-label="집열기 종류 비교 닫기" onClick={(event)=>{event.stopPropagation();setActiveDetail(0);}}>×</button><header><span>SOLAR THERMAL COLLECTOR TYPES</span><b>집열기 형식 비교</b><small>선택한 형식의 전용 성능식이 시간별 집열량과 필요 면적 계산에 적용됩니다.</small></header><div className="collector-type-grid"><article className="evacuated"><div className="collector-illustration" aria-hidden="true"><i/><i/><i/><i/><span/></div><div><em>EVACUATED TUBE</em><h4>진공관형 집열기</h4><p>이중 유리관 사이의 진공층이 대류·전도 열손실을 줄입니다. 외기와 작동유체의 온도차가 큰 조건에서도 상대적으로 유리합니다.</p><dl><div><dt>구조</dt><dd>독립 진공관 배열</dd></div><div><dt>성능식</dt><dd>η₀−a₁ΔT/G−a₂ΔT²/G</dd></div></dl></div></article><article className="flat"><div className="collector-illustration" aria-hidden="true"><span/><b>{Array.from({length:6}).map((_,i)=><i key={i}/>)}</b></div><div><em>FLAT PLATE</em><h4>평판형 집열기</h4><p>투명 덮개 아래 흡수판과 유로가 하나의 평판 모듈로 구성됩니다. 단순한 구조와 넓은 유효 흡수면이 특징입니다.</p><dl><div><dt>구조</dt><dd>흡수판·유로 일체형</dd></div><div><dt>성능식</dt><dd>Fᵣ[(τα)G−UₗΔT]</dd></div></dl></div></article></div><p className="collector-note">두 형식은 동일 식의 계수만 바꾸는 구조가 아닙니다. 진공관형은 2차 효율곡선, 평판형은 열제거계수·광학효율·열손실계수를 이용한 유효 집열식으로 각각 계산합니다.</p></aside>}
...
function ResultOverview({summary,design,onReset}:{summary:CalculationSummary;design:Design;onReset:()=>void}) {
  ...
  const [summaryRegionKey,setSummaryRegionKey]=useState(summary.primaryKey);
  const [heatmapMode,setHeatmapMode]=useState<"abs"|"reg">("abs");
  const primary=summary.regions.find(item=>item.key===summaryRegionKey)||summary.regions[0];
  ...
  return <main className="result-screen">
    ...
    <section className="ld-output-strip"><header><span>CALCULATED LD OPERATING CONDITIONS</span><h2>산정된 LD 운전조건과 장치 용량</h2></header><div><article><small>LiCl 초기 농도</small><b>{n(best.solutionConcentration).toFixed(1)}%</b></article><article><small>평균 L/G</small><b>{n(best.lgRatioMean||best.lgRatio).toFixed(2)}</b></article><article><small>평균 제습부 용액온도</small><b>{n(best.absorberSolutionTempMean||best.absSolutionTemp).toFixed(1)} °C</b></article><article><small>평균 재생부 용액온도</small><b>{n(best.regeneratorSolutionTempMean||best.regenTemp).toFixed(1)} °C</b></article><article><small>병렬 모듈</small><b>{n(best.absorberModules).toFixed(0)} / {n(best.regeneratorModules).toFixed(0)}대</b></article></div><p>앞 숫자는 제습부, 뒤 숫자는 재생부입니다. 제습부는 건물 외기량, 재생부는 목표 농도에서의 수분 배출 부하로 독립 산정합니다. 운전 중에는 설치 대수를 초과하지 않습니다.</p>{best.regenSizingMode==="load"&&<div style={{marginTop:16}}><article><small>재생 설계 외기량</small><b>{Math.round(n(best.regenDesignAirM3h)).toLocaleString()} m³/h</b><small>제습 대비 {n(best.regenDesignAirRatio).toFixed(2)}배</small></article><article><small>재생 고정 L/G · 필요 / 설치 대수</small><b>{n(best.regenFixedLg).toFixed(2)} · {n(best.regenDesignRequiredModules)} / {n(best.regeneratorModules)}대</b><small>{best.regenDesignLimited?"설계 상한 또는 재생 불가조건: 부족 확인 필요":"목표 농도 기준 수분부하로 산정"}</small></article><article><small>덕트 단면 추정 · 5 m/s 가정</small><b>{n(best.regenDesignDuctAreaM2).toFixed(2)} m²</b></article><article><small>팬동력 추정 · 300 Pa / η 60%</small><b>{n(best.regenDesignFanKW).toFixed(2)} kW</b><small>펌프·모터·압력손실 상세 검토 별도</small></article></div>}</section>
    <section className="result-dashboard">
      ...
      <article className="chart-card monthly-card"><header><div><span>MONTHLY ENERGY FLOW</span><h2>재생열 요구량과 태양열 공급</h2><small>두 막대는 동일한 0점과 동일한 에너지 축을 사용합니다. 같은 값은 같은 높이로 표시됩니다.</small></div><div className="chart-legend"><i/>재생열 요구량 <i/>태양열 실사용</div></header><div className="bar-axis-layout"><div className="bar-y-title">에너지 (kWh)</div><div className="bar-y-ticks">{monthlyTicks.map(value=><span key={value}>{Math.round(value).toLocaleString()}</span>)}</div><div className="monthly-bars">{monthly.map((item,index)=><div className="month-group" key={index}><div><i data-value={n(item.load)>0?(n(item.load)>=1000?`${(n(item.load)/1000).toFixed(1)}k`:Math.round(n(item.load))):undefined} style={{height:`${n(item.load)/monthlyMax*100}%`}} title={`요구량 ${n(item.load).toFixed(0)} kWh`}/><i data-value={n(item.solar)>0?(n(item.solar)>=1000?`${(n(item.solar)/1000).toFixed(1)}k`:Math.round(n(item.solar))):undefined} style={{height:`${n(item.solar)/monthlyMax*100}%`}} title={`태양열 ${n(item.solar).toFixed(0)} kWh`}/></div><span>{String(item.month||index+1).replace("월","")}월</span></div>)}</div><div className="bar-x-title">월</div></div></article>
...
function GeographicAnalysis({regions,selectedKey,onSelect}:{regions:CalculationRegion[];selectedKey:string;onSelect:(key:string)=>void}) {
  ...
  const labelOffsets:Record<string,[number,number]>={seoul_epw:[18,-16],incheon_tmyx:[-24,16],berlin_tmyx:[-20,-15],manila_tmy:[-19,-15]};
...
function WeatherChart({region,regions,selectedKey,onSelect}:{region:CalculationRegion;regions:CalculationRegion[];selectedKey:string;onSelect:(key:string)=>void}) {
  const [month,setMonth]=useState<number|null>(null),[day,setDay]=useState<number|null>(null);
  useEffect(()=>{setMonth(null);setDay(null)},[region]);
  const hourly=region.weatherHourly||[];
  const selected=hourly.filter(r=>r.month===month);
  const rows=month===null?(region.weatherMonthly||[]).map(r=>({...r,key:r.month,label:r.month+"월"})):day===null?Array.from(new Set(selected.map(r=>r.day))).sort((a,b)=>a-b).map(d=>{
    const hours=selected.filter(r=>r.day===d),duration=hours.reduce((s,r)=>s+r.duration,0);
    return {key:d,label:d+"일",outdoorTemp:hours.reduce((s,r)=>s+r.outdoorTemp*r.duration,0)/duration,outdoorHumidity:hours.reduce((s,r)=>s+r.outdoorHumidity*r.duration,0)/duration,irradiation:hours.reduce((s,r)=>s+r.irradiance*r.duration/1000,0)};
  }):selected.filter(r=>r.day===day).map(r=>({...r,key:r.hour,label:r.hour+"시",irradiation:r.irradiance}));
  const period=month===null?"월별":day===null?month+"월 일별":month+"월 "+day+"일 시간별";
  const solarUnit=day===null?"kWh/m²":"W/m²",averageLabel=month===null?"월평균":day===null?"일평균":"시간별";
  const drill=(key:number)=>{if(month===null){setMonth(key);setDay(null)}else if(day===null)setDay(key)};
  const canDrill=day===null&&hourly.length>0;
  ...
  const tb=bounds(temps),hb=bounds(humidity),sb=bounds(solar);
  const leftMin=Math.floor(Math.min(...temps,...humidity,0)/5)*5;
  const leftMax=Math.max(leftMin+5,Math.ceil(Math.max(...temps,...humidity,1)/5)*5);
  const rightMax=Math.max(day===null&&month!==null?1:50,Math.ceil(Math.max(...solar,1)/(day===null&&month!==null?1:50))*(day===null&&month!==null?1:50));
  const leftTicks=Array.from({length:5},(_,index)=>leftMax-(leftMax-leftMin)*index/4);
  const rightTicks=Array.from({length:5},(_,index)=>rightMax*(4-index)/4);
  const yLeft=(value:number)=>88-(value-leftMin)/(leftMax-leftMin)*76;
  const yRight=(value:number)=>88-value/rightMax*76;
  const x=(index:number)=>rows.length>1?index/(rows.length-1)*100:50;
  const tempPoints=temps.map((value,index)=>`${x(index)},${yLeft(value)}`).join(" "),humidityPoints=humidity.map((value,index)=>`${x(index)},${yLeft(value)}`).join(" ");
  return <article className="chart-card weather-card"><header><div><span>WEATHER &amp; SOLAR INPUT</span><h2>{period} 날씨 및 일사 데이터</h2><small>{averageLabel} 외기온도·절대습도와 {day===null?"기간 누적 집열면 일사량":"집열면 일사강도"}입니다. 월 → 일 → 시간 순으로 선택하세요.</small></div>{regions.length>1&&<label className="weather-region-select">지역<select value={selectedKey} onChange={e=>onSelect(e.target.value)}>{regions.map(item=><option value={item.key} key={item.key}>{item.label.split(" · ")[0]}</option>)}</select></label>}</header><nav className="concentration-breadcrumb" aria-label="기상 그래프 기간"><button onClick={()=>{setMonth(null);setDay(null)}}>월별 전체</button>{month!==null&&<button onClick={()=>setDay(null)}>{month}월</button>}{day!==null&&<button disabled>{day}일</button>}</nav>{!hourly.length&&<p>상세 기상 추이는 설계 계산을 다시 실행하면 확인할 수 있습니다.</p>}<div className="weather-combo-legend"><span><i className="temp"/><span>외기온도<small>{averageLabel} · 왼쪽 축</small></span><b>{tb.min.toFixed(1)}–{tb.max.toFixed(1)} °C</b></span><span><i className="humidity"/><span>절대습도<small>{averageLabel} · 왼쪽 축</small></span><b>{hb.min.toFixed(1)}–{hb.max.toFixed(1)} g/kgDA</b></span><span><i className="solar"/><span>집열면 일사량<small>{day===null?"기간 누적":"시간별"} · 오른쪽 축</small></span><b>{sb.min.toFixed(0)}–{sb.max.toFixed(0)} {solarUnit}</b></span></div><div style={{overflowX:"auto"}}><div className="weather-axis-layout" style={{minWidth:month===null?700:Math.max(900,rows.length*40)}}><div className="weather-y-title">외기온도 (°C) · 절대습도 (g/kgDA)</div><div className="weather-y-ticks" aria-hidden="true">{leftTicks.map(value=><span key={value}>{Number.isInteger(value)?value:value.toFixed(1)}</span>)}</div><div className="weather-combo-plot"><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-label={`${region.label} 월별 날씨 및 일사 복합 그래프`}><defs><linearGradient id="weatherSolarFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#ff8279" stopOpacity=".42"/><stop offset="1" stopColor="#ff8279" stopOpacity=".04"/></linearGradient></defs>{[12,31,50,69,88].map(v=><line key={v} x1="0" x2="100" y1={v} y2={v} className="weather-gridline"/>)}{rows.map((_,index)=><line key={`v${index}`} x1={x(index)} x2={x(index)} y1="12" y2="88" className="weather-gridline weather-gridline-v"/>)}{solar.map((value,index)=>{const width=rows.length?32/rows.length:3;const barY=yRight(value);return <rect key={index} x={x(index)-width/2} y={barY} width={width} height={88-barY} className="weather-solar-bar"><title>{rows[index]?.label} · 일사량 {value.toFixed(1)} {solarUnit}</title></rect>})}<polyline points={tempPoints} className="weather-temp-line"/><polyline points={humidityPoints} className="weather-humidity-line"/>{temps.map((value,index)=><circle key={`t${index}`} cx={x(index)} cy={yLeft(value)} r="1.35" className="weather-temp-dot"><title>{rows[index]?.label} · 외기온도 {value.toFixed(1)} °C</title></circle>)}{humidity.map((value,index)=><circle key={`h${index}`} cx={x(index)} cy={yLeft(value)} r="1.35" className="weather-humidity-dot"><title>{rows[index]?.label} · 절대습도 {value.toFixed(1)} g/kgDA</title></circle>)}</svg><div className="weather-month-axis" style={{gridTemplateColumns:`repeat(${rows.length},1fr)`}}>{rows.map(row=><button disabled={!canDrill} onClick={()=>drill(row.key)} key={row.key}>{row.label}</button>)}</div><div className="weather-x-title">{month===null?"월":day===null?"일":"시각 (24시간)"}</div></div><div className="weather-y-ticks weather-y-ticks-right" aria-hidden="true">{rightTicks.map(value=><span key={value}>{Math.round(value)}</span>)}</div><div className="weather-y-title weather-y-title-right">집열면 일사량 ({solarUnit})</div></div></div></article>;
}
...
function ConcentrationChart({monthly,drilldown,region}:{monthly:Array<Record<string,number|string>>;drilldown:ConcentrationMonth[];region:string}) {
  ...
  const plotMin=Math.floor((Math.min(rawMin,36.4)-.2)*10)/10,plotMax=Math.ceil((Math.max(rawMax,39)+.2)*10)/10,range=Math.max(plotMax-plotMin,.5);
  ...
  const areaPoints=`0,88 ${meanPoints} 100,88`;
  ...
  const plotWidth=selectedMonth===null?760:selectedDay===null?Math.max(980,rows.length*42):Math.max(820,rows.length*44);
  const periodMean=rows.reduce((sum,item)=>sum+item.mean,0)/rows.length;
  const levelLabel=selectedMonth===null?"연간":selectedDay===null?`${selectedMonth}월`:`${selectedMonth}월 ${selectedDay}일`;
  ...
  return <article className="chart-card concentration-card"><header><div><span>LD SOLUTION STATE · {region.split(" · ")[0]}</span><h2>{title}</h2><small>{description}</small></div><nav className="concentration-breadcrumb" aria-label="농도 그래프 상세 단계"><button className={selectedMonth===null?"selected":""} onClick={()=>{setSelectedMonth(null);setSelectedDay(null)}}>연간</button>{selectedMonth!==null&&<><i>›</i><button className={selectedDay===null?"selected":""} onClick={()=>setSelectedDay(null)}>{selectedMonth}월</button></>}{selectedDay!==null&&<><i>›</i><button className="selected">{selectedDay}일</button></>}</nav></header><div className="concentration-kpis"><div><small>표시 기간</small><b>{levelLabel}</b></div><div className="primary"><small>평균 농도</small><b>{periodMean.toFixed(2)} <i>wt%</i></b></div><div><small>관측 범위</small><b>{rawMin.toFixed(2)}–{rawMax.toFixed(2)} <i>wt%</i></b></div><div><small>실험식 적용범위</small><b>36.4–39.0 <i>wt%</i></b></div></div><div className="concentration-chart-shell"><div className="concentration-chart-meta"><div className="concentration-legend"><span className="mean"/>평균 농도 <span className="range"/>최소–최대 <span className="valid"/>실험식 적용범위</div><small>포인트를 눌러 월 → 일 → 시간 순으로 확인</small></div><div className="concentration-axis-unit">LiCl 농도 · wt%</div><div className="concentration-layout"><div className="concentration-y-axis">{ticks.map(value=><span key={value} style={{top:`${y(value)}%`}}>{value.toFixed(1)}</span>)}</div><div className="concentration-plot" style={{minWidth:plotWidth}}><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-label={`${region} ${title}`}><defs><linearGradient id="concentrationArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#4ee7f0" stopOpacity=".24"/><stop offset="100%" stopColor="#4ee7f0" stopOpacity="0"/></linearGradient></defs>{ticks.map(value=><line key={value} x1="0" x2="100" y1={y(value)} y2={y(value)} className="concentration-grid"/>)}<rect x="0" y={y(39)} width="100" height={Math.max(0,y(36.4)-y(39))} className="concentration-valid-zone"/><polygon points={areaPoints} className="concentration-area"/><polygon points={bandPoints} className="concentration-band"/><polyline points={meanPoints} className="concentration-line"/>{rows.map((item,index)=><circle tabIndex={selectedDay===null?0:undefined} role={selectedDay===null?"button":undefined} onClick={()=>selectPoint(item.key)} onKeyDown={event=>{if(event.key==="Enter"||event.key===" ")selectPoint(item.key)}} key={item.key} cx={x(index)} cy={y(item.mean)} r="1.55" className={`concentration-dot${selectedDay===null?" clickable":""}`}><title>{item.label} · 평균 {item.mean.toFixed(2)} wt% · 범위 {item.min.toFixed(2)}–{item.max.toFixed(2)} wt%</title></circle>)}</svg><div className={`concentration-x-axis axis-${selectedMonth===null?"months":selectedDay===null?"days":"hours"}`} style={{gridTemplateColumns:`repeat(${rows.length},minmax(32px,1fr))`}}>{rows.map(item=><button disabled={selectedDay!==null} onClick={()=>selectPoint(item.key)} key={item.key}><i/>{item.label}</button>)}</div><div className="concentration-x-title">{selectedMonth===null?"월":selectedDay===null?"일 (날짜)":"시각 (24시간)"}</div></div></div></div></article>;
}
...
function InlineDetail({type,region,onClose}:{type:"dehum"|"unmet"|"area";region:CalculationRegion;onClose:()=>void}) {
  ...
  const [traceTime,setTraceTime]=useState<string[]|null>(null);
  useEffect(()=>{setSelectedUnmetEvents(new Set());setTraceTime(null)},[type,region]);
  const toggleUnmetEvent=(index:number)=>{setTraceTime(null);setSelectedUnmetEvents(current=>{const next=new Set(current);if(next.has(index))next.delete(index);else next.add(index);return next})};
  ...
  if(type==="unmet") {
    ...
    const events=((trend.events as Array<Record<string,unknown>>)||[]);
    const selectedIndex=Array.from(selectedUnmetEvents).at(-1);
    const selectedTime=selectedIndex===undefined?null:String(events[selectedIndex]?.time||"");
    ...
    const condition=(value:unknown,unit:string)=>value===null||value===undefined||!Number.isFinite(Number(value))?"—":Number(value).toFixed(unit?1:2)+unit;
    ...
    const xIndexes=events.length?Array.from(new Set([0,Math.round((events.length-1)/3),Math.round((events.length-1)*2/3),events.length-1])):[];
    return <section className="inline-detail"><DetailHeader index="02" title="목표 제습 미충족 추이" region={region.label} onClose={onClose}/><div className="trace-action"><div><p id="trace-help">아래 점 또는 표의 행을 선택한 뒤 버튼을 누르면, 선택한 시간의 앞뒤 30분을 포함해 내부 계산을 수행하고 60초 단위로 집계합니다. 연속되거나 겹치는 구간은 합쳐 중복을 제거합니다. 예: 14시·15시 선택 → 13:30~16:30. 떨어진 구간은 별도로 표시합니다.</p><strong aria-live="polite">{selectedTime?"선택 "+selectedUnmetEvents.size+"개 · "+region.label:"먼저 분석할 점을 선택하세요."}</strong></div><button type="button" disabled={!selectedTime} aria-describedby="trace-help" onClick={()=>setTraceTime(Array.from(selectedUnmetEvents).map(index=>String(events[index].time)).sort())}>선택 시점의 60초별 상세 계산</button></div><div className="unmet-summary"><div><b>{n(trend.totalHours).toFixed(1)} h</b><small>상한 초과시간</small></div><div><b>{n(trend.averageHumidityExcess).toFixed(2)} g/kgDA</b><small>평균 초과량</small></div><div><b>{n(trend.maxHumidityExcess).toFixed(2)} g/kgDA</b><small>최대 초과량</small></div></div><div className="unmet-chart"><div className="unmet-y-title">급기 절대습도 (g/kgDA)</div><div className="unmet-y-axis">{ticks.map(tick=><span key={tick} style={{bottom:`${y(tick)}%`}}>{tick.toFixed(1)}</span>)}</div><div className="unmet-points">{ticks.map(tick=><em key={tick} style={{bottom:`${y(tick)}%`}}/>)}{events.map((item,index)=><button type="button" key={index} className={selectedUnmetEvents.has(index)?"selected":""} aria-pressed={selectedUnmetEvents.has(index)} aria-label={`${item.time} 급기 절대습도 ${n(item.supplyHumidity).toFixed(2)} g/kgDA`} title={`${item.time} · ${n(item.supplyHumidity).toFixed(2)} g/kgDA`} onClick={()=>toggleUnmetEvent(index)} style={{left:`${x(index)}%`,bottom:`${y(n(item.supplyHumidity))}%`}}/>)}<span className="unmet-upper-line" style={{bottom:`${y(accepted)}%`}}>허용상한 {accepted.toFixed(1)} g/kgDA</span><span className="unmet-average-line" style={{bottom:`${y(averageSupply)}%`}}>미충족 평균 {averageSupply.toFixed(2)} g/kgDA</span></div><div className="unmet-x-axis">{xIndexes.map(index=><span key={index} style={{left:`${x(index)}%`}}>{String(events[index]?.time||"").slice(5,16)}</span>)}</div><div className="unmet-x-title">발생 시각 (월-일 시:분)</div></div>{traceTime&&<SubstepTrace key={traceTime.join(",")} request={region.traceRequest} time={traceTime[0]} times={traceTime}/>}<p className="ld-event-note">LD 조건은 해당 시간 중 실제 가동 구간 기준입니다. 제습 L/G·용액온도 및 재생 용액온도는 가동시간 가중평균, 재생 L/G는 누적 용액·공기 질량비입니다. 정지 또는 이전 결과의 미기록 값은 —로 표시합니다. 표를 좌우로 밀어 운전조건을 확인하세요.</p><div className="event-table event-table-detailed"><div><b>발생시각</b><b>외기조건</b><b>LiCl 농도</b><b>급기 절대습도</b><b>상한 초과</b><b>제습 L/G</b><b>재생 L/G</b><b>제습 용액온도</b><b>재생 용액온도</b></div>{events.map((item,index)=><button type="button" key={index} className={selectedUnmetEvents.has(index)?"selected":""} aria-pressed={selectedUnmetEvents.has(index)} onClick={()=>toggleUnmetEvent(index)}><span>{String(item.time||"").slice(5)}</span><span className="outdoor-condition"><b>{n(item.outdoorTemp).toFixed(1)} °C</b><small>{n(item.outdoorHumidity).toFixed(2)} g/kgDA</small></span><span>{eventConcentration(item)>0?`${eventConcentration(item).toFixed(2)} wt%`:"–"}</span><span>{n(item.supplyHumidity).toFixed(2)} g/kgDA</span><span>+{n(item.humidityExcess).toFixed(2)}</span><span>{condition(item.absLg,"")}</span><span>{condition(item.regLg,"")}</span><span>{condition(item.absTemp," ℃")}</span><span>{condition(item.regTemp," ℃")}</span></button>)}</div></section>;
  }
