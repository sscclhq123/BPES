...
export default function LdFlowPlots({rows,rawRows=rows,reference,xTitle,onSelect,compact=false,designAir,airLimit=3}:{rows:FlowRow[];rawRows?:FlowRow[];reference?:number;xTitle:string;onSelect?:(i:number)=>void;compact?:boolean;designAir?:number;airLimit?:number}) {
 ...
 <div><Plot title="제습 설계 풍량 대비 재생 외기량" unit="배" yMax={Math.max(airLimit,...airMultiples.filter((v):v is number=>v!==null))} xTitle={xTitle} rows={rows} onSelect={onSelect} series={[{name:average?"재생 풍량 배수 · 정지 포함":"재생 풍량 배수 · 가동 중",color:"#098ea4",values:airMultiples},{name:`설정 상한 ${airLimit.toFixed(1)}배`,color:"#c74b40",dash:true,values:rows.map(()=>airLimit)},{name:"제습 설계 풍량 1배",color:"#6c7f86",dash:true,values:rows.map(()=>1)}]}/><p className="ld-flow-note">배수 = 재생 외기 질량유량 ÷ 제습 설계 공기 질량유량 ({flowFormat(airBase)} kg/s). 제습이 정지해도 분모는 고정입니다. 위 유량 표시 기준을 함께 적용합니다.{!airBase&&" 기준 풍량이 없는 이전 결과입니다. 설계 계산을 다시 실행하세요."}</p></div>
