...
export type FlowData={version:number;rows:[string,number,number,number,number|null,number|null,number|null,number|null,number|null,number|null,number|null][]};
export default function LdFlowDiagnostics({data,region,reference,request}:{data?:FlowData;region:string;reference?:number;request?:Record<string,unknown>}) {
 ...
 <LdFlowPlots rows={rows} rawRows={filtered} designAir={Number(request?.airflow)>0?Number(request?.airflow)/3600*1.2:all.find(r=>r.absAir!=null&&r.absAir>0)?.absAir??undefined} airLimit={Number(request?.regenMaxAirRatio)||3} reference={reference} xTitle={!month?"월":!day?"일":"시각 (24시간)"} onSelect={select}/>
