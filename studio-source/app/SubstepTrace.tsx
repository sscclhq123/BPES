...
export default function SubstepTrace({request,time}:{request?:Record<string,unknown>;time:string}){
 ...
 <p>외기는 이 구간 내 고정 입력입니다. 급기습도는 각 내부 구간의 부분운전을 포함한 평균, 농도는 구간 끝 상태입니다. 제습 제한 시 미처리 공기도 평균에 포함됩니다. 그래프는 전체 60분을 표시합니다. 상세 결과표는 좌우로 밀어 확인할 수 있습니다.</p>
 <div className="trace-plots"><Plot title="급기 절대습도" unit="g/kgDA" duration={trace.durationSeconds} limit={trace.upper} series={[{name:"내부 구간 평균",color:"#098ea4",points:staircase("supplyHumidity")},{name:`시간 평균 ${f(trace.hourSupplyHumidity)}`,color:"#53666f",points:[[0,trace.hourSupplyHumidity],[trace.durationSeconds,trace.hourSupplyHumidity]]}]}/>
 ...
 </div><details><summary style={{cursor:"pointer",padding:12}}>계산식과 {steps.length}개 구간 결과표 펼치기</summary>
